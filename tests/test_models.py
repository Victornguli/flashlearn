...
class TestModels:
	...
	def test_deck_model(self, user, decks):
		child = Deck(
			name = 'Recursion', description='Recursive Algorithms', user = user, parent_id = decks[0].id)
		child.save()
		assert child in user.decks, "Child deck should be available in the user's decks"
		assert child in decks[0].children, "Child deck should be available in parent's children"
	...
	def test_card_model(self, user, decks):
		test_card = Card(
			...
			user_id = user.id, deck_id = decks[1].id)
		test_card.save()
